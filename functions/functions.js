...
class FunctionsUtils {
    ...
    async Wo_GetMessageByID(ctx,id) {
    ...
    async Wo_GetPostReactions(ctx,object_id, col = "post",type = '') {
       ...
       var reactions_count = 0;
       var name = col+'_id';
        if (object_id == false || typeof object_id !== 'number') {
            return "";
        }
       if (type == 'blog') {
            var result = await ctx.wo_blog_reaction.findAll({
    ...
    async liksMatch(text = '',hasHTML = false) {
        let link_regex = new RegExp('(http\:\/\/|https\:\/\/|www\.)([^\ ]+)', 'gi');
        let linkSearch = text.match(link_regex)
        if (linkSearch && linkSearch.length > 0) {
              hasHTML = true;
              for (let linkSearchOne of linkSearch) {
                  let matchUrl = striptags(linkSearchOne)
                  let syntax = '[a]' + escape(matchUrl) + '[/a]'
                  text = text.replace(link_regex, syntax)
              }
        }
        
        return { msg: text, hasHTML };
    }

    async mentionMatch(text = '',hasHTML = false) {
        let mention_regex = new RegExp('@([A-Za-z0-9_]+)', 'gi');
          let mentionSearch = text.match(mention_regex)
          if (mentionSearch && mentionSearch.length > 0) {
              hasHTML = true;
              for (let mentionSearchOne of mentionSearch) {
                  ...
                  if (mention) {
                      ...
                      text = text.replace(mention_regex, match_replace)
                  }
              }
          }
    ...
    async hashTagsMatch(text = '',hasHTML = false) {
        let hashTagSearch = text.match(/#([^`~!@$%^&*\#()\-+=\\|\/\.,<>?\'\":;{}\[\]* ]+)/gi)
          if (hashTagSearch && hashTagSearch.length > 0) {
              hasHTML = true
              for (let hashTagSearchOne of hashTagSearch) {
                  ...
                  text = text.replace(/#([^`~!@$%^&*\#()\-+=\\|\/\.,<>?\'\":;{}\[\]* ]+)/gi, replaceString)
    ...
    async cleanBR(text = '') {
        text = await this.sanitizeJS(text);
        text = text.replace("\r\n", " <br>");
        text = text.replace("\n\r", " <br>");
